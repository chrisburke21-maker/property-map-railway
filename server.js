...
const map = L.map('map', { doubleClickZoom: false, renderer: canvasRenderer }).setView([34.5, -111.9], 7);
...
let pricingMarkers = L.markerClusterGroup({
  maxClusterRadius: 40,
  disableClusteringAtZoom: 14,
  chunkedLoading: true,
  chunkInterval: 100,
  ...
  iconCreateFunction: function(cluster) {
    ...
    return L.divIcon({
      html: '<div style="background:#d946ef;color:#fff;border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;border:2px solid #86198f">' + count + '</div>',
      className: '',
      iconSize: [36, 36]
    });
  }
...
function buildPricingPopup(pm) {
  ...
  if (pm.date) popup += '<tr><td style="color:#64748b">Date Observed</td><td style="text-align:right">' + String(pm.date).substring(0, 10) + '</td></tr>';
  popup += '</table>';
...
function renderPricingMarkers(data) {
  ...
  data.forEach(function(pm) {
    ...
    // Lazy popup
    marker.on('click', (function(entry) {
      return function() {
        this.bindPopup(buildPricingPopup(entry), { maxWidth: 400 }).openPopup();
      };
    })(pm));

    batch.push(marker);
  });
  pricingMarkers.addLayers(batch);
}
...
function renderMarkers(data) {
  ...
  data.forEach(p => {
    ...
    // Lazy popup — build HTML only on click
    marker.on('click', (function(prop) {
      return function(e) {
        if (drawMode) return;
        var radiusVal = parseFloat(document.getElementById('radius').value);
        if (radiusVal > 0) {
          radiusCenter = { lat: prop.gps_lat, lng: prop.gps_long, apn: prop.apn };
          applyFilters();
        } else {
          this.bindPopup(buildPropertyPopup(prop), { maxWidth: 400 }).openPopup();
        }
      };
    })(p));

    // Double-click → open property in Supabase
    marker.on('dblclick', (function(prop) {
      return function(e) {
        L.DomEvent.stopPropagation(e);
        window.open('https://supabase.com/dashboard/project/xanfkjitudcpduodadgu/editor?schema=public&table=properties&filter=id%3Aeq%3A' + prop.id, '_blank');
      };
    })(p));
...
async function generateAndCache() {
  ...
  // Pre-compress for fast serving
  cachedGzip = zlib.gzipSync(cachedHTML);
  console.log('Generated HTML at ' + generatedAt + ' (' + Math.round(cachedHTML.length / 1024) + 'KB raw, ' + Math.round(cachedGzip.length / 1024) + 'KB gzip)');
}
...
async function startServer() {
  ...
  const server = http.createServer(async (req, res) => {
    if (req.url === '/' || req.url === '/index.html') {
      ...
      if (ae.includes('gzip') && cachedGzip) {
        res.writeHead(200, {
